...
pub const IO_Uring = struct {
    ...
    /// Queues an multishot recv on a socket.
    ///
    ...
    pub fn recv_multishot(
        ...
        var sqe = try self.get_sqe();
        io_uring_prep_rw(.RECV, sqe, fd, 0, 0, 0);
        sqe.rw_flags = flags;
        sqe.flags |= linux.IOSQE_BUFFER_SELECT;
    ...
    /// Sets the amount of bounded and unbounded workers. Use 0 to retrieve the current value.
    /// The array fields are [2]i32{bounded_count, unbounded_count}
    ...
    pub fn register_iowq_max_workers(self: *IO_Uring, workers: *[2]i32) !void {
        assert(self.fd >= 0);
        const res = linux.io_uring_register(
            self.fd,
            .REGISTER_IOWQ_MAX_WORKERS,
            @as(*const anyopaque, @ptrCast(workers)),
            @as(u32, @intCast(workers.len)),
        );
        try handle_registration_result(res);
    }
...
pub const BufferGroup = struct {
    ...
    pub fn init(
        ...
        assert(buffers.len == @as(usize, @intCast(buffers_count)) * @as(usize, @intCast(buffer_size)));
        ...
        while (i < buffers_count) : (i += 1) {
            const start = @as(usize, @intCast(buffer_size)) * i;
    ...
    pub fn get(self: *BufferGroup, buffer_id: u16) []u8 {
        const head = @as(usize, @intCast(self.buffer_size)) * buffer_id; // TODO make member proper size
...
pub fn io_uring_setup_buf_ring(fd: os.fd_t, entries: u16, group_id: u16) !*align(mem.page_size) linux.io_uring_buf_ring {
    ...
    const mmap_size = @as(usize, @intCast(entries)) * @sizeOf(linux.io_uring_buf);
    const mmap = try os.mmap(
        null,
        mmap_size,
        os.PROT.READ | os.PROT.WRITE,
...
pub fn io_uring_prep_multishot_recv(
    sqe: *linux.io_uring_sqe,
    fd: os.fd_t,
    buffer: []u8,
    flags: u32,
...
pub fn io_uring_prep_multishot_recvmsg(
    sqe: *linux.io_uring_sqe,
    fd: os.fd_t,
    msg: *os.msghdr,
    flags: u32,
